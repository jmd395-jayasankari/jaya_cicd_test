 ...
 /* This model calculates flags for revenue events by assessing product growth and decline across various periods (monthly, quarterly, yearly, and year-to-date), joining revenue data with customer and product lifecycle information to identify cross-sell, upsell, and downsell activities. */
    ...
    WITH product_grew AS (
        ...
        SELECT
            ...
            , {{ get_dimension_from_table('period_revenue', 'entity') }} 
            , {{ get_dimension_from_table('period_revenue', 'other') }} 
            , month_roll
            , revenue_type
            , arr
            , arr_lm
    ...
    , ranked_product AS (
        ...
        SELECT 
            ...
            , {{ get_dimension_from_table('period_revenue', 'entity', 'p1') }} 
            , {{ get_dimension_from_table('period_revenue', 'other', 'p1') }} 
            , p1.revenue_type  
            , p1.arr
            , p1.arr_lm
            , p1.arr_l3m
...
-- Uncomment this - If you want to calculate the Intermittent_churn , Winback , deactivation, reactivaiton helper columns
...
-- If you want to calculate the Intermitten Churn and winback - change the destination CTE
...
SELECT
    ...
    , {{ get_dimension_from_table('monthly_revenue', 'entity') }} 
    , {{ get_dimension_from_table('monthly_revenue', 'other') }}
    , month_roll
    , revenue_type

    -- Remove 0  - If you want to calculate the Intermitten Churn and Winback 
    , 0  AS winback_helper
    ...
    , 0  AS l3m_reactivation_helper
    , 0  AS l3m_winback_helper
    , CASE 
    ...
    , 0  AS ltm_reactivation_helper
    , 0  AS ltm_winback_helper
    , CASE 
    ...
    , 0  AS ytd_reactivation_helper
    , 0  AS ytd_winback_helper
    , CASE 
