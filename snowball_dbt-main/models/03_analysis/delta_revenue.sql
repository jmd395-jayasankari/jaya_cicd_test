...
WITH get_arr_business_flag AS (
    ...
    SELECT
        ...
        , {{ get_dimension_from_table('monthly_revenue', 'key', 'p1', exclude_list=['MONTHLY_REVENUE_KEY']) }} 
        , {{ get_dimension_from_table('monthly_revenue', 'level', 'p1') }} 
        , p1.month_roll
        , p1.revenue_type
        , p1.arr
        , p1.arr_lm
...
, filling_delta AS (
    ...
    SELECT
        ...
        period_revenue_key
        , {{ get_dimension_from_table('monthly_revenue', 'key', exclude_list=['MONTHLY_REVENUE_KEY']) }} 
        , {{ get_dimension_from_table('monthly_revenue', 'level') }} 
        , month_roll
        , revenue_type
