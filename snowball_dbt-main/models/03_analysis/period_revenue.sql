...
, get_revenue_lags AS (
    ...
    SELECT
        ...
        , {{ get_dimension_from_table('monthly_revenue', 'entity', 'a') }} 
        ...
        , a.volume
        , a.revenue_type
        , COALESCE(LAG(a.arr, {{var('monthly')}}) OVER (PARTITION BY a.monthly_revenue_key, a.revenue_type ORDER BY a.month_roll), 0)         AS arr_lm   -- Monthly Revenue Lag
        , COALESCE(LAG(a.arr, {{var('quarterly')}}) OVER (PARTITION BY a.monthly_revenue_key, a.revenue_type ORDER BY a.month_roll), 0)       AS arr_l3m  -- Quarterly Revenue Lag
        , COALESCE(LAG(a.arr, {{var('yearly')}}) OVER (PARTITION BY a.monthly_revenue_key, a.revenue_type ORDER BY a.month_roll), 0)          AS arr_ltm  -- Yearly Revenue Lag
